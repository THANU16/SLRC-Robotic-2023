  ...
  switch (mode)
    {
      ...
      case 5:
         ...
         calculateError();
         if (mode == 5) mazeEnd();
         else goAndTurn (YES, LEFT, 90); // or it is a "T" or "Cross"). In both cases, goes to LEFT
         break;
      case 3:
         runExtraInch();
         readIRSensors();
         calculateError();
         if (mode == 4) goAndTurn (YES, RIGHT, 90);
         break;
      case 2:
         goAndTurn (NO, LEFT, 90);
         break;
      case 1:
         followLine();
         break;
...
void goAndTurn(String Extra, String directionM, int degree) {
  int adjGoAndTurn = 50;                        //  motorPIDcontrol();
  if (Extra == "YES") {
         forward(adjGoAndTurn); 
//        delay(3000);
  } else {
        forward(adjGoAndTurn+20);
  }
