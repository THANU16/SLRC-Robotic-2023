...
int leastConstant= 20;

const float timeFact = 5.83; // valid for 1 degree turn and motor Vcc=5.3V and max speedCar


//UltraSensorTHings
int trigPin[] = {30,36,34,38, 40}; //Trig Pins
int echoPin[] = {31,37,35, 39, 41}; //EchoPins
//38, 39 right
//40, 41 left
...
//    }
  ...
  do {
      turnRight(30);
        distanceCalc();
      } while (distance[2] < leastdistance || distance[0] < leastdistance);
     ...
     do {
      turnLeft(30);
        ...
        distanceCalc();
      } while (distance[1] < leastdistance || distance[0]<leastdistance);

      initialStartCar(30);
   }

  if ((distance[1] <leastConstant) && (distance[2] <leastConstant) && (distance[0] >leastConstant)) {
//    stopM(1000);
  initialStartCar(30);
    do {
    forward(30);
    ...
    distanceCalc();
    } while ((distance[0]>leastConstant) && (distance[1] >8) && (distance[2]>8)) ;
...
//      turnRight(80);
    ...
    do {
     ...
     turnRight(60);
     if(distance[2] <leastConstant || distance[0] <leastConstant ){
      do {
        turnRight(30);
     
        distanceCalc();
      } while (distance[2] >leastConstant && distance[0] >leastConstant);
     }
...
//      turnLeft(80);
    ...
    distanceCalc();
     ...
     turnLeft  (60);
     if(distance[1] <leastConstant || distance[0] <leastConstant ){
      do {
        turnLeft(30);
     
        distanceCalc();
      } while (distance[1] >leastConstant && distance[0] >leastConstant);
     }
